...
def diff_eq(SimulationParameters, SimulationFlow, i_particle_pos_vel, time):
    ...
    result = np.zeros(
        (SimulationParameters.num_particles, 2*dim_number),
        dtype='float32')

    st = SimulationParameters.st
    beta = SimulationParameters.beta

    material_derivative_result = material_derivative(
        SimulationFlow=SimulationFlow,
    ...
    result[:, dim_number:2*dim_number] = (
        beta * material_derivative_result
        + (1/st) * (SimulationFlow.flow(position, time) - velocity)
    )
...
def RK4_step(SimulationParameters, SimulationFlow, i_particle_pos_vel, time):
    ...
    k2 = diff_eq(
        SimulationParameters,
        SimulationFlow,
        i_particle_pos_vel + B[1] * k1 * time_step,
        time + A[1] * time_step)
    k3 = diff_eq(
        SimulationParameters,
        SimulationFlow,
        i_particle_pos_vel + B[2] * k2 * time_step,
        time + A[2] * time_step)
    k4 = diff_eq(
        SimulationParameters,
        SimulationFlow,
        i_particle_pos_vel + B[3] * k3 * time_step,
        time + A[3] * time_step)

    # Calculate updated position and velocity
    result = (
        i_particle_pos_vel
        + (CH[0] * k1 + CH[1] * k2 + CH[2] * k3 + CH[3] * k4)
        * time_step
    )
...
def run_simulation(
    ...
    particle_acc, particle_pos_vel = generate_tracking_arrays(
    ...
    for i in range(num_steps):
        ...
        particle_acc[:, i][:, 0:dim_number] = diff_eq(
            SimulationParameters=SimulationParameters,
            SimulationFlow=SimulationFlow,
            i_particle_pos_vel=i_particle_pos_vel,
        ...
        if i < num_steps - 1:
            ...
            for k in range(len(SimulationRegion.boundary_conditions)):
                ...
                particle_pos_vel[:, i+1] = boundary_condition_function(
                    SimulationRegion=SimulationRegion,
