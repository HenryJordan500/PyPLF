"""
Utility functions for creating flow fields,
generating particle tracking arrays,
...
def generate_tracking_arrays(initial_particles, simulation_steps, dim_number):
    """
    Initialize particle tracking arrays for
    ...
    particle_pos_vel = np.zeros(
        (len(initial_particles), simulation_steps, dim_number*2),
        dtype='float32')
    particle_acc = np.zeros(
        (len(initial_particles), simulation_steps, dim_number),
        dtype='float32')
...
def save_data(particle_acc, particle_pos_vel, dim_number, save_path):
    ...
    acc = fil.create_dataset(
        "acceleration",
        data=particle_acc,
        dtype='float16')
    vel = fil.create_dataset(
        "velocity",
        data=particle_pos_vel[:, :, dim_number:2*dim_number],
        dtype='float16')
    pos = fil.create_dataset(
        "position",
        data=particle_pos_vel[:, :, 0:dim_number],
        dtype='float16')
    del acc, vel, pos
    fil.close()
...
def extract_data(save_path, extract=None):
    ...
    if extract is None:
        ...
        raise KeyError(
            'Extract key must be set to either '
